...
import { ChevronDown, ChevronUp, Clock, Lock, Timer, CheckCircle2 } from 'lucide-react';
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-white/5 bg-[#0b0f19]/50">
                    <div className="flex flex-col divide-y divide-white/5">
                      {module.lessons.map((lesson) => {
                        ...
                        const isSequenceLocked = !lesson.is_completed && !isNextLesson;
                        ...
                        const isCompletelyLocked = isSequenceLocked || isTimeLocked;

                        // FIX: Now Live disappears once completed. Continue appears if progress > 0.
                        ...
                        return (
                          <div key={lesson.id} className={`flex flex-col md:flex-row md:items-center justify-between p-5 gap-4 transition-colors ${!isCompletelyLocked && isNextLesson ? 'bg-blue-500/[0.03]' : isCompletelyLocked ? 'opacity-80' : 'hover:bg-white/[0.02]'}`}>
                            <div className="flex items-start md:items-center gap-4">
                              <div className="mt-0.5 md:mt-0 shrink-0">
                                {lesson.is_completed ? <CheckCircle2 size={20} className="text-green-500" /> : isTimeLocked && !isSequenceLocked ? <div className="w-6 h-6 rounded-full bg-amber-500/10 flex items-center justify-center text-amber-500"><Timer size={14} /></div> : isNextLesson && !isTimeLocked ? <div className="w-5 h-5 rounded-full border-2 border-blue-500 flex items-center justify-center"><div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse" /></div> : <Lock size={18} className="text-slate-700" />}
                              </div>
                              <div>
                                <h4 className={`font-medium ${lesson.is_completed ? 'text-slate-300' : isNextLesson && !isTimeLocked ? 'text-white font-bold' : 'text-slate-500'}`}>{lesson.title}</h4>
                                <div className={`flex items-center gap-1.5 text-xs mt-1 ${isCompletelyLocked ? 'text-slate-700' : 'text-slate-500'}`}><Clock size={12} /> {lesson.estimated_time} • Video</div>
                              </div>
                            </div>
                            <div className="pl-9 md:pl-0 shrink-0">
                              ...
                              >
                                {isSequenceLocked ? 'Locked' : isTimeLocked ? `Premieres ${formattedTime}` : lesson.is_completed ? 'Review' : isNowLive ? 'NOW LIVE' : hasStarted ? 'Continue' : 'Start'}
                              </button>
                            </div>
                          </div>
                        );
