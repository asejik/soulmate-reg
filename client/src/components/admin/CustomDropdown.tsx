...
export const CustomDropdown: React.FC<CustomDropdownProps> = ({ value, options, onChange, icon: Icon, align = 'left', className = '' }) => {
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button type="button" onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center gap-2 bg-[#13132b] border border-white/10 rounded-xl px-4 py-3 hover:bg-white/5 transition-colors focus:outline-none">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: 0.15 }} className={`absolute top-full mt-2 ${align === 'right' ? 'right-0' : 'left-0'} w-full min-w-[200px] bg-[#1a1a3a] border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50 max-h-60 overflow-y-auto`}>
            {options.map((opt) => (
